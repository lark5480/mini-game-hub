...
## 验收标准
- [x] npm run build 零错误
- [~] 终局后「查看棋谱」：点条目棋盘回退该步 + from/to 高亮；◀▶ 逐手；▶ 自动播放三档速度；⏮ 回开局（功能已交付，播放自停已修复，用户实测待最终确认）
...
| 收尾 | Codex | 任务交付完成：feat b8faf9f + fix 51ebc32 + chore 6d00748，已 push（dev）；state.json 复位（current_task=null）；剩余冒烟项为交付后用户验收项 | 无 |
