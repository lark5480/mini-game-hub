...
| 17 | 2026-08-14 | 象棋 AI 修复 review（2026-08-14-xiangqi-ai-fix） | 搜索内重复局面裁决与视图裁决语义不一致：negamax 把局面第 3 次出现一律判最后走子方负（±MATE），视图对双方闲着循环判和 → AI 在可循环求和局面误判必胜且主动避和；完整对齐需搜索内做将/杀/捉分类（成本高），暂维持现状。2026-08-14 更新（2026-08-14-xiangqi-ai-perpetual-check-fix）：罚分已加强为 2×MATE + 按被将归属 + 第 2 次重复提前惩罚将军方（AI 不主动长将判负）；双方闲着循环判和与长捉分类的语义对齐仍遗留 | 待处理 |
...
| 20 | 2026-08-14 | 象棋提前出手 review（2026-08-14-xiangqi-ai-early-exit） | AI 走子固定 400ms 展示延迟可降到 250-300ms（提前出手已生效后，固定延迟占总响应时间比例上升）；纯 UX 微调 | 已完成(2026-08-14-xiangqi-ai-ux-polish：400→250ms) |
| 21 | 2026-08-14 | 象棋 AI 细节优化 review（2026-08-14-xiangqi-ai-ux-polish） | 对 AI 战绩统计（localStorage 胜/负/和 + 难度面板展示），激励重复游玩 | 待处理 |
| 22 | 2026-08-14 | 象棋 AI 细节优化 review（2026-08-14-xiangqi-ai-ux-polish） | 续局功能：xiangqi_record 目前只写不读（死数据）；读取并恢复 AI 模式对局需扩展存档字段（mode/difficulty/aiSide/gameStarted）并回放 moves 重建局面 | 待处理 |
| 23 | 2026-08-14 | 象棋 AI 细节优化 review（2026-08-14-xiangqi-ai-ux-polish） | 开局库扩充 3-5 条主流变化谱（当前 7 条主线多局雷同）；与 #18（简单模式库命中弱化）联动 | 待处理 |
