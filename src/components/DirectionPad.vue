    ...
    <template v-if="layout === 'cross'">
      <div class="pad-row" v-if="showUp">
        <button
          ...
          @keydown.enter.prevent="emit('up', false)"
          @keydown.space.prevent="emit('up', false)"
        >
      ...
      <div class="pad-row">
        <button
          ...
          @keydown.enter.prevent="emit('left', false)"
          @keydown.space.prevent="emit('left', false)"
        >
        ...
        <button
          ...
          @keydown.enter.prevent="emit('down', false)"
          @keydown.space.prevent="emit('down', false)"
        >
        ...
        <button
          ...
          @keydown.enter.prevent="emit('right', false)"
          @keydown.space.prevent="emit('right', false)"
        >
...
const emit = defineEmits<{
  (e: 'up' | 'down' | 'left' | 'right', isRepeat: boolean): void
}>()
...
function startHold(dir: Dir) {
  ...
  if (props.repeat) {
    repeatTimers[dir] = setInterval(() => emit(dir, true), REPEAT_MS)
  }
}
